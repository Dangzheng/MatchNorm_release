...
class DGCNN(nn.Module):
    ...
    def forward(self, x, mn_list):
        ...
        x = F.relu(mn2(self.conv1(x)))
        x1 = x.max(dim=-1, keepdim=True)[0]

        x = F.relu(mn3(self.conv2(x)))
        x2 = x.max(dim=-1, keepdim=True)[0]

        x = F.relu(mn4(self.conv3(x)))
        x3 = x.max(dim=-1, keepdim=True)[0]

        x = F.relu(mn5(self.conv4(x)))
        x4 = x.max(dim=-1, keepdim=True)[0]

        x = torch.cat((x1, x2, x3, x4), dim=1)

        x = F.relu(mn6(self.conv5(x))).view(batch_size, -1, num_points)
        return x

class BPNet_pure_mn(nn.Module):
    def __init__(self, emb_net=DGCNN, pointer_net=Transformer):
        super(BPNet_pure_mn, self).__init__()
        self.emb_nn = emb_net()
